...
С помощью брандмауэра запрещаем весь трафик, кроме сквида. 

Приходится отказаться от прав администратора для исключения возможности снятия фильтрации...
...
```
ssl_crtd.exe -c -s C:\squid\etc\bump\ssldb 
```
(ВАЖНО: каталога ssldb существовать не должно, иначе вылезет ошибка!)
...
Данный скрипт работает только на русифицированной версии Windows. Там подробное описание каждой команды. Будут ошибки пишите

Скрипт понижает права текущего пользователя и запрашивает новый пароль админстратора. Рекомендую для начальной настройки ввести пустой пароль Администратора и ,если все заработает, то ввести случайный пароль, который забываем
